...
const KiemTra = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const dashRes = await learningApi.getDashboard();
  ...
  return (
    <div>
      ...
      <PageSectionHeader
        tone="emerald"
        title="Kiểm tra — làm bài tự đánh giá kiến thức TTATGT"
      />

      <div className="flex flex-col md:flex-row md:flex-wrap">
        {list.map((item, index) => {
          const hasMonthiTest = !!item.link_test;
          return (
            <div key={item._id} className="w-full md:basis-1/4 px-2 my-3">
              ...
              >
                ...
                <div className="relative z-10 flex flex-wrap items-center justify-end gap-2">
                  <NavLink
                    to={`/kiem-tra/${item._id}`}
                    className="inline-flex items-center rounded-full bg-white/20 px-3 py-1.5 text-xs font-semibold text-white no-underline ring-1 ring-white/30 backdrop-blur-sm"
                  >
                    <span>Chọn chuyên đề</span>
                    <ArrowRight className="ml-1 h-4 w-4" />
                  </NavLink>
                  {hasMonthiTest && (
                    <NavLink
                      to={`/tu-kiem-tra/${item.link_test}`}
                      className="inline-flex items-center rounded-full bg-white/30 px-3 py-1.5 text-xs font-semibold text-white no-underline ring-1 ring-white/40 backdrop-blur-sm"
                    >
      ...
      {list.length === 0 && (
        <p className="text-center text-gray-500 px-4 py-6">
          Chưa có nội dung kiểm tra được hiển thị.
        </p>
      )}
    </div>
  );
