...
export default function AdminDarboard() {
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      Cookies.remove("refreshToken_thitracnghiem");
      Cookies.remove("accessToken_thitracnghiem");
      navigate("/login");
    } catch (error) {
      console.log(error);
    ...
    <div className="bg-gray-300">
        ...
        <AppBar position="static">
          <Toolbar>
            ...
            <p className="flex-grow">
              <span className="md:inline text-[7px] md:text-[14px] hidden uppercase">Tuyên truyền, đánh giá nhận thức pháp luật về Trật tự an toàn giao thông</span>
              <span className="md:hidden text-[12px] md:text-[14px] uppercase">Cục cảnh sát giao thông - Bộ công an</span>
              {/* <span className="md:hidden inline text-sm">PM thi trắc nghiệm CA Hưng Yên</span> */}
            </p>
      ...
      <Drawer anchor={"left"} open={state} onClose={toggleDrawer(false)}>
        <div className="min-w-[300px] p-4 mt-2">
          <div className="flex items-center space-x-2">
            ...
            <p className="text-xl text-gray-800 font-bold">Menu chức năng hệ thống</p>
          </div>
          ...
          <ul className="mt-2 px-4">
            ...
            {/* {roles && roles.includes("xem đơn vị") && (
              <li className="text-md my-2 hover:font-bold transition-all py-2 border-b">
                <NavLink
                  to="/admin/quan-ly-don-vi"
                  style={({ isActive }) => (isActive ? activeStyle : undefined)}
                >
                  <div className="flex items-center space-x-2">
      ...
      <div className="bg-[#1976d2] py-4 pl-4 mb-2 text-center text-white text-sm mx-2 mr-2 p-2 border-t-black border-t-[1px] shadow-md">
        ...
        <h5 className="font-light">
          Ứng dụng công nghệ thông tin trong công tác tuyên truyền, phổ biến, giáo dục pháp luật về trật tự an toàn giao thông
        </h5>
      </div>
    </div>
  );
